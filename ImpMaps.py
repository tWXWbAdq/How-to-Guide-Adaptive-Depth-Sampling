...
def main():
    if args.mult_factor is None:                     # define scaling parameter for ImpMaps (to extend the dynamic range to 2^16)
        if args.err_type == 'RMSE':
            args.mult_factor = 100.0
        if args.err_type == 'REL':
            args.mult_factor = 50000.0

    model = fusionnet_def(args.model_path)      # define FusionNet model

    impmap_type = "err" + args.err_type + "_perc" + str(args.samp_perc) + "_iter" + str(args.rand_iters) + '_m' + str(args.mult_factor)  # define ImpMaps folder name
    for set1 in sets:
        print('set: ' + set1)
        ...
        for inx in tqdm(range(len(left_test))):                                 # for each RGB + GT couple
            ...
            if not os.path.isfile(ImpMap_savepath):                             # if this file does not already exists
                ...
                ImpMap *= args.mult_factor                                           # scale ImpMap before saving
